...
//  const FOB_CYCLES_COUNT = 4;
    ...
    public function processData()
    {
        ...
        /*fopen('post.log', 'r');
        $file = file_get_contents('post.log');
        ob_start();
        var_dump($_POST);
        $post = ob_get_clean();
...
//        }
        ...
        if(isset($_POST['ETH_PACKET_FW_UPDATE_ANSWER']))
        {
            session_start();
            $header = $_SESSION['header'];
        }

        //СОХРАНИТЬ ЗАГОЛОВОК В СЕССИЮ
...
//                }
            ...
            if ($deviceOptions['last_dispatch'] < $deviceOptions['updated_at'])
            {
...
//        header("HTTP/1.1 202 Good job");
    ...
    public function updateFirmware($header)
    {
        ...
        }elseif($_POST['ETH_PACKET_FW_UPDATE_ANSWER'] == "FW full OK")
        {
            ...
            $_SESSION['count'] = $_SESSION['count'] - 512;
            $firmwarePart = substr($contents, $_SESSION['count'], 256);
            echo $_SESSION['header'] . $firmwarePart;
            $_SESSION['count'] += 256;
        }else{
        ...
        fopen('firmware.log', 'r');
        $file = file_get_contents('firmware.log');
        /*if(isset($_POST['ETH_PACKET_FW_UPDATE_ANSWER']))
        {
        ...
        if(isset($_SESSION['count']))
        {
            $data = $_SESSION['count']/256;
            file_put_contents("firmware.log", "\n" . $data . $file);
        }
